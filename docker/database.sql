...
create or replace trigger changeFoodName
after update on food
referencing
    new as newRow
    old as oldRow
for each row when (newRow.foodName <> oldRow.foodName)
    begin
        update food_recipes set foodName = :newRow.foodName where foodName = :oldRow.foodName;
    end;
/

create or replace trigger changeRecipeName
after update on recipes
referencing
    new as newRow
    old as oldRow
for each row when (newRow.recipeName <> oldRow.recipeName)
    begin
        update food_recipes set recipeName = :newRow.recipeName where recipeName = :oldRow.recipeName;
    end;
/
...
insert into food_recipes (recipeName, foodName, amount, units) values ('plain burger', 'burger bun', 1, null);
...
insert into food_recipes (recipeName, foodName, amount, units) values ('cheeseburger', 'burger bun', 1, null);
...
insert into food_recipes (recipeName, foodName, amount, units) values ('bacon cheeseburger', 'burger bun', 1, null);
